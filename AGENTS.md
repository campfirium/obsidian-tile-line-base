...
## 文档维护（T0000 路线图）
- `docs/tasks/T0000.md` 保持原有“## 任务：……”逐块格式，按字段顺序记录 status、分支、说明等信息。
- 将完成的事项标记为 `status：done`，并同步更新 `statusChanged` 的时间戳；未启动或暂停的任务沿用 `todo`/`canceled`。
- 已有内容不要修改，可补全空字段，但避免改动既有描述或说明。
- `分支` 字段只填写单一的四位任务编号（如 `0001`、`0034`）；若涉及多个任务，拆分为独立条目后分别记录。
- 所有 bug 修复（原 `fix/` 分支）统一设置 `分类：bug`，同时让 价值 / 成本（m） / 优先级 字段留空。
- 分类字段只能使用现有分类（如 基础、核心、核心功能、核心交互、健壮性、体验优化、便利 等），不新增自定义类别。
- 增补新任务或补录既有成果时，按现有字段格式填写；暂未确定的评估项可以留空，待确认后再补充。
