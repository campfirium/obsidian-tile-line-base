...
在修复过程中发现并同时修复了另一个相关问题：

- `src/grid/AgGridAdapter.ts`
  - 修改 `handleDeleteKey()` 方法 (L548-593)
  - **问题**: 在过滤视图中按 Delete/Backspace 键无法删除单元格内容
  - **原因**: 使用 `findRowNodeByBlockIndex()` 查找行节点，该方法通过遍历所有节点查找，但在过滤视图中只能遍历过滤后的行
  - **解决**: 直接使用 `gridApi.getFocusedCell()` 和 `getDisplayedRowAtIndex()` 获取当前聚焦的单元格和行节点，支持过滤视图
