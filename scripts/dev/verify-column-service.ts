...
import { cloneColumnState } from '../../src/grid/column/ColumnStateManager';
...
const tests: TestCase[] = [
	...
	},
	{
		...
		run: () => {
			...
			const clone = cloneColumnState(source);
	...
	},
	{
		...
		run: () => {
			...
			assert(
				resizeLogs.refreshHeader === 0,
				'Initial resize should complete without forcing a header refresh'
			);
			assert(resizeLogs.refreshCells.length === 0, 'Initial resize should not force a cell refresh');
			...
			assert(
				resizeLogs.refreshHeader === 1,
				'Clamping oversized columns should refresh the header once'
			);
			assert(
				(resizeLogs.refreshCells.at(-1) as { force?: boolean } | undefined)?.force === true,
				'Clamping should force a cell refresh'
			);
		}
	}
];
