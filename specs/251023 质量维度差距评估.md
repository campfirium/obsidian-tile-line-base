...
### 键盘路径补充（草案）
| 交互场景 | 进入方式 | 操作键 | 预期反馈 | 当前状态 | 后续动作 |
| --- | --- | --- | --- | --- | --- |
| StatusCellRenderer 状态切换 | 网格内使用箭头或 Tab 聚焦状态列单元格 | Space / Enter | 状态在 `todo <-> done` 间切换，`aria-label` 同步更新 | 已更新代码（待冒烟） | 手动补充键盘冒烟记录，验证后回填日期 |
| StatusCellRenderer 状态菜单展开 | 保持单元格聚焦 | Shift+F10 / ContextMenu 键 | 打开状态菜单并自动聚焦第一项 | 已更新代码（待冒烟） | 验证聚焦顺序并确认视觉高亮，记录结果 |
| StatusCellRenderer 菜单导航 | 菜单已展开 | Up / Down、Enter、Esc | 上下移动高亮，Enter 确认切换，Esc 关闭并归还焦点 | 已更新代码（待冒烟） | 安排 NVDA 朗读回归，确认高亮朗读 |
| ColumnEditorModal 初始焦点 | 通过工具栏按钮或列头菜单打开 | Modal 打开时 | 第一个 Setting 控件获得焦点并有视觉提示 | 焦点逻辑已实现（待验证） | 手动确认初始焦点与视觉反馈，记录触发入口 |
| ColumnEditorModal 快速关闭 | Modal 已打开 | Esc | Modal 关闭且焦点回到触发按钮 | Esc 行为已实现（待验证） | QA 模板追加该用例并验证焦点回退 |
| FilterViewModals 条件导航 | Modal 内已有多个条件块 | Tab / Shift+Tab | 焦点按视觉顺序循环，标签文本被朗读 | Tab 顺序复核未见异常 | 追加读屏标签与焦点约束 TODO，留待后续任务 |

### a11y 验证计划
- 阶段一：键盘路径冒烟验证，覆盖上表所有场景；工具：Chrome 现版本 + 键盘，可复用 AG Grid 官方可访问性调试指南；输出：每条路径的“步骤/预期/结果”记录附加到本 specs。
- 阶段二：屏幕阅读器朗读对照，优先使用 NVDA 2024.2（Windows）与 macOS VoiceOver；关注菜单高亮朗读、Modal 标签与提示文本；输出：问题截图或录音 + 结论，必要时创建 `fix/Txxxx-a11y` 分支跟进。
- 阶段三：可视化反馈回归，检查焦点环、对比度及状态提示，与 Obsidian 原生任务列表对比；输出：更新 `docs/tasks/` 对应任务条目，列出需人工确认的控件清单。
