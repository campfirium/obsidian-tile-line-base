...
const QUICK_FILTER_INPUT_DEBOUNCE_MS = 125;
...
export class GlobalQuickFilterController {
	...
	private handleInput(value: string): void {
		...
		this.pendingInputValue = normalized;
		this.updateIndicators(normalized);
		this.schedulePendingInput();
	}
	...
	private schedulePendingInput(): void {
		...
		if (this.inputDebounceTimer !== null) {
			ownerWindow.clearTimeout(this.inputDebounceTimer);
		}
		this.inputDebounceTimer = ownerWindow.setTimeout(() => {
			this.inputDebounceTimer = null;
			this.flushPendingInput();
		}, QUICK_FILTER_INPUT_DEBOUNCE_MS);
	}
	...
	private flushPendingInput(): void {
		...
		const normalized = this.pendingInputValue;
